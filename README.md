...
[HackIDLE-NIST-Coder Model](https://ollama.com/etgohome/hackidle-nist-coder) is an AI model trained specifically on cybersecurity topics and NIST frameworks.
...
You run the HackIDLE-NIST-Coder model through Ollama.
...
*Note: The prompt above is a simple example. When you write your own prompts, be very specific and include guardrails (argparse, pagination, error handling, control mapping, etc.) so the AI generates safer, more complete scripts.*

**3.** Save the generated script locally

*Note: Always review and add guardrails to AI-generated scripts. The model produces working code, but not production-safe or compliance-ready code.*
...
john.morris | ID: AIDAV2H00TS0BOJID0003 | ARN: arn:aws:iam::300066700000:user/john.morris | Created: 2025-09-10 14:17:09
...
```
...
| -------- | ------------------ | ------------------------------------------------------------- | ------------------------------------------------------- |
| **AC-2** | Account Management | Ensure accounts are created, managed, and reviewed per policy.| Lists all IAM users for visibility and control testing. |
